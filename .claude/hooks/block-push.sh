...
# DEZOGIF_ALLOW_PUSH=1 is set in the environment of the launched command.
#
...
#
# Override: prefix the command with `DEZOGIF_ALLOW_PUSH=1` (the user types this,
...
if printf '%s' "$cmd" | grep -qE '(^|[[:space:]])DEZOGIF_ALLOW_PUSH=1([[:space:]]|$)'; then
  exit 0
fi
...
If the user just said push/PR, prefix the command with:
    DEZOGIF_ALLOW_PUSH=1 <your command>
EOF
  exit 2
fi
