...
#
# Override: DEZOGIF_ALLOW_MAIN_WRITE=1 (only when the user has explicitly
...
# Override.
if printf '%s' "$cmd" | grep -qE '(^|[[:space:]])DEZOGIF_ALLOW_MAIN_WRITE=1([[:space:]]|$)'; then
  exit 0
fi
...
authorized to land work on main):
    DEZOGIF_ALLOW_MAIN_WRITE=1 <your command>
EOF
  exit 2
done < <(printf '%s\n' "$cmd" | tr ';&|' '\n')
