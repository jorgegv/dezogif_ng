...
#
# Override: prefix with DEZOGIF_ALLOW_AMEND=1 if the user explicitly asks to amend.
...
if printf '%s' "$cmd" | grep -qE '(^|[[:space:]])DEZOGIF_ALLOW_AMEND=1([[:space:]]|$)'; then
  exit 0
fi
...
Override (only when the user explicitly says to amend):
    DEZOGIF_ALLOW_AMEND=1 git commit --amend ...
EOF
  exit 2
fi
