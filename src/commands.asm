...
cmd_init:
    ...
    ;
    ; THIS CALL MUST STAY ABOVE THE prgm_state WRITE TWO INSTRUCTIONS BELOW, and
    ; that ordering is the whole safety of it: copper_break_arm reads prgm_state
    ; to tell a first attach - nothing loaded, nothing running, so nothing that
    ; can own the Copper - from a re-attach to a debuggee that may have
    ; installed a list of its own. Move it below and it reads PRGM_LOADING every
    ; time, the guard is silently inert, and a reconnect destroys the debuggee's
    ; raster effects irrecoverably, because the list is write-only. It is also
    ; why this is here and not on the resume path. See ui.asm's copper_break_arm.
